...
#define immB() do { *imm = (SEXT(BITS(i, 31, 31), 19) << 19) | (BITS(i, 7, 7) << 11) | (BITS(i, 30, 25) << 5) | (BITS(i, 11, 8) << 1); } while(0)
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, if (SEXT(src1, 32) >= SEXT(src2, 32)) s->dnpc = s->pc + SEXT(imm, 12));
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, if (SEXT(src1, 32) < SEXT(src2, 32)) s->dnpc = s->pc + SEXT(imm, 12));
