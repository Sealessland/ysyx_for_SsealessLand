...
class Decode extends Module {
  ...
  // 手动筛选真正的指令定义文件
  val opcodesDir = os.pwd / "riscv-opcodes"
  
  // 只选择 extensions 目录下的特定文件，避免包含其他非指令文件
  val extensionsDir = opcodesDir / "extensions"
  val wantedFiles = Seq("rv32_i", "rv64_i", "rv_i", "rv_m", "rv64_m", "rv32_m")
  
  val filteredFiles = wantedFiles.map(extensionsDir / _).filter(os.exists)

  // 创建解析器需要的元组
  val opcodeFileTuples = filteredFiles.map { p =>
    (
      p.last,
      os.read(p),
      true,  // 认为这些都是 ratified 的
      false  // 不是 custom 的
    )
  }

  // 创建 argLut，并转换为所需的格式
  val argLutFromDb = org.chipsalliance.rvdecoderdb.argLut(opcodesDir, Seq.empty)
  val argLutMap: Map[String, (Int, Int)] = argLutFromDb.map { case (name, arg) =>
    name -> (arg.msb, arg.lsb)
  }

  // 使用新的、非废弃的方法进行解析
  val instTable = org.chipsalliance.rvdecoderdb.parser.parse(opcodeFileTuples, argLutMap)
